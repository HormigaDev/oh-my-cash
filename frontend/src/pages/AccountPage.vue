          ...
          <q-card-section>
            <div class="account-card__heading">
              ...
              <div
                ...
                ><p>{{ t("account.profile.description") }}</p></div
              >
            </div>
          ...
          <q-card flat class="account-card">
            <q-card-section>
              <div class="account-card__heading">
                <q-icon name="palette" />
                <div
                  ><h2>{{ t("account.appearance.title") }}</h2
                  ><p>{{ t("account.appearance.description") }}</p></div
                >
              </div>
              <ThemePalettePicker />
              <p class="account-appearance__mode-label">{{
                t("theme.mode")
              }}</p>
            ...
            <q-card-section>
              <div class="account-card__heading">
                ...
                <div
                  ...
                  ><p>{{ t("account.security.description") }}</p></div
                >
              </div>
              <q-form class="account-form" @submit="savePassword">
                <q-input
                  ...
                  ><template #append
                    ><q-icon
                      ...
                      @click="showCurrent = !showCurrent" /></template
                ></q-input>
                <q-input
                  ...
                  ><template #append
                    ><q-icon
                      ...
                      class="account-password-toggle"
                      @click="showNext = !showNext" /></template
                ></q-input>
                <q-input
                ...
                <div class="account-form__actions"
                  ><q-btn
                    ...
                    no-caps
                    color="primary"
                    :label="t('account.security.change')"
...
const filteredTimezones = computed(() =>
  timezoneValues
    .filter(value =>
      value.toLowerCase().includes(timezoneQuery.value.toLowerCase())
    )
...
const passwordRules = [
  (value: string) =>
    value.length >= 12 || t("account.validation.passwordLength")
];
const confirmRules = [
  (value: string) =>
    value === password.next || t("account.validation.passwordMatch")
];
...
function message(error: unknown) {
  ...
  if (isApiError(error) && error.code === "INVALID_CREDENTIALS")
    return t("account.errors.currentPassword");
...
.account-card :deep(.q-card__section) {
  padding: 1.4rem;
}
...
.account-appearance__mode-label {
  margin: 1rem 0 0.45rem !important;
  color: var(--omc-color-text-secondary) !important;
  font-weight: 650;
}
.account-appearance__hint {
  margin: 0.55rem 0 0 !important;
  color: var(--omc-color-text-muted) !important;
  font-size: 0.75rem;
  line-height: 1.45;
}
.account-appearance__error {
  margin-top: 0.75rem;
}
